...
/*							LogWriter.stdErrLogln("level=" + level + ", slot=" + slot + ", targetlevel=" + targetLevel + ", slotPos=" +
		...
		if (minLevel == -1) { // read from the arrays pulled from H1
			...
			//bigBuffI[outBuffPos++] = (int)keys.pointer[inputPos];
		...
		if (outBuffPos == 10*this->numFieldsPerBlock) { //changed
			...
			fwrite(bigBuffI,sizeof(int),10*this->numFieldsPerBlock,this->file); //TODO: debug: check this place again and look for possible bugs

			outFilePos += 10*this->numFieldsPerBlock;
			outBuffPos = 0;
...
void ArrayHeapExtMem::load (int level, int slot){
	...
	while ( slotPositions[level][slot] < endPos )  {
		 //TODO: debug: so when this code is executed for the second time, apparently there are some values wrong, and they end up being 0, and the problem is probably where these values are stored
		// rather than here. The place is probably the function store.
		//TODO: debug: the wrong value is first encountered at position 512, which is position 170 of the values written (513/3). All the values after this point make no sense.
		// 512 is probably not a random number, it might be a hint to help me debug this. I have to figure which function is writing this wrong value. Every fwrite to file, is a possible target for debugging.
		val1 = buffI[i++];
		val2 = buffI[i++];
...
void ArrayHeapExtMem::removeMin(){
	...
	if (!which) {
		...
		if (cntOnHeap[lvl][slot] == 0) { //TODO: debug: when this code executes for the second time, the next value is wrong( it`s a 0).
...
/*	//print everything for debug, in no particular order
	for(int k=0;k<h->H2->size();k++){
		printf("%f\n",h->H2->heap->at(k).key);
	}
	return false;*/

	if(verbose)
	printf("Removing and asserting... \n"); //TODO: there is currently a bug somewhere here that includes a bunch of zeros between correct values, the first occurance is at 11043
	while (!h->isEmpty() && isOk) {
/*		if (i==11042){ // so here everything looks fine
			printf("It`s here.\n");
			isOk=false;
			for(int k=0;k<h->H2->size();k++){
				printf("%f\n",h->H2->heap->at(k).key);
		...
		if (i==11043){ // here a lot of zeros are introduced into the heap out of nowhere
			printf("It`s here.\n");
			//isOk=false;
			for(int k=0;k<h->H2->size();k++){
				printf("%f\n",h->H2->heap->at(k).key);
