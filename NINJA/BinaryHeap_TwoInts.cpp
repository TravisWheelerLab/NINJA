...
BinaryHeap_TwoInts::BinaryHeap_TwoInts() {
    ...
    this->heap->reserve(BinaryHeap_TwoInts::DEFAULT_CAPACITY);
...
BinaryHeap_TwoInts::BinaryHeap_TwoInts(const int *val1s, const int *val2s, Float keys) {
    this->heap = new std::vector<ints2float>();
    this->heap->reserve(BinaryHeap_TwoInts::DEFAULT_CAPACITY);
    std::make_heap(this->heap->begin(), this->heap->end(), compare2IntsFloat);
...
BinaryHeap_TwoInts::BinaryHeap_TwoInts(const int *val1s, const int *val2s, Float keys, int maxCapacity) {
    this->heap = new std::vector<ints2float>();
    this->heap->reserve(maxCapacity);
    std::make_heap(this->heap->begin(), this->heap->end(), compare2IntsFloat);
    for (int i = 0; i < keys.length; i++) {
...
void BinaryHeap_TwoInts::buildAgain(const int *val1s, const int *val2s, Float keys) {
    makeEmpty();
    for (int i = 0; i < keys.length; i++) {
        ints2float aux{};
        aux.first = val1s[i];
...
void BinaryHeap_TwoInts::insert(int val1, int val2, float key) const {
    ints2float aux{};
    aux.first = val1;
    aux.second = val2;
    aux.key = key;
...
void BinaryHeap_TwoInts::deleteMin() const {
    std::pop_heap(this->heap->begin(), this->heap->end(), compare2IntsFloat);
    this->heap->pop_back();
}


bool BinaryHeap_TwoInts::isEmpty() const {
    return this->heap->empty();
}

int BinaryHeap_TwoInts::size() const {
    return this->heap->size();
}

void BinaryHeap_TwoInts::makeEmpty() const {
    if (this->heap == nullptr) return;
    this->heap->clear();
}

void BinaryHeap_TwoInts::chopBottomK(int *val1s, int *val2s, Float keys) const { //no idea if it works
    int k = keys.length;
    size_t pos;
    for (int i = 0; i < k; i++) {
    ...
    for (int l = 1; l < 6; l++) {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            for (int i = 0; i < 5000; i++) {
                int test = (int) aux->heap->front().key;
                int test2 = int_aux[i];
                assert(test2 == test);
                aux->deleteMin();
            }
