...
#pragma omp parallel for
        for (int i = 0; i < this->K; i++)
            for (int j = i + 1; j < this->K; j++) {
                ...
                distances[i][j - i - 1] = 100 * (int) (((100000000 * this->distCalc->calc(i, j)) + 50) /
                                                       100); // this gets the same rounding I have in the distance writer code
...
//		printf("-----------");
            ...
            for (int i = 0; i < cnt; i++) {
                distances[i][cnt - i - 1] =
                        100 * (int) ((((100000000 * std::atof(lineString.substr(pos, length).c_str()))) + 50) / 100);
...
void DistanceReader::readAndWrite(std::string **names,
                                  FILE *outFile) { //possibly wrong, the else part, perhaps the distance calculator class as well
...
#pragma omp parallel for
    for (int i = 0; i < this->K; i++) {
        for (int j = i + 1; j < this->K; j++) {
            //distances[i][j-i-1] = this->distCalc->testDifferenceCluster(i,j);
            distances[i][j - i - 1] = this->distCalc->calc(i,
                                                           j); // this gets the same rounding I have in the distance writer code
        }
    }
    this->write(outFile, distances, names);
}

void DistanceReader::write(FILE *outFile, double **distances, std::string **names) const {
...
float DistanceReader::atoi(char *in, int end) {
    ...
    while (pos >= 0) {
        if (in[pos] == '-') {
            ...
            fprintf(stderr,
                    "Unable to convert integer from invalid char array. Position + %d, char = '%c', numberString = '%s'",
                    pos, in[pos], in);
            Exception::critical();
        } else {
