...
bool compare3Ints(ints3 x, ints3 y) { return (x.key > y.key); }
...
BinaryHeap_IntKey_TwoInts::BinaryHeap_IntKey_TwoInts(int *val1s,
                                                     const int *val2s,
                                                     Int keys) {
    this->heap = new std::vector<ints3>();
    this->heap->reserve(this->DEFAULT_CAPACITY);
...
bool BinaryHeap_IntKey_TwoInts::isEmpty() { return this->heap->empty(); }

int BinaryHeap_IntKey_TwoInts::size() { return this->heap->size(); }
...
bool BinaryHeap_IntKey_TwoInts::binHeapTwoTest(bool verbose) {
    ...
    for (int l = 1; l < 6; l++) {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            if (verbose)
                printf("Binary Heap Two Ints Test %d completed successfully.\n",
                       k);
        }
    }
