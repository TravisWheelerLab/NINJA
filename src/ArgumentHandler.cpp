...
if (i < argc-1){
    if(!y.compare("default")){
        ...
        fprintf(stderr,"Invalid method: available methods are  'default',
'inmem', and 'extmem'."); Exception::critical();
    }
    i++;
}else{
    ...
    Exception::critical();
            ...
            printf("Option --method is not implemented in this version of "
                   "Ninja.\n");
            ...
            if (i < argc - 1) {
                if (!y.compare("d")) {
                    ...
                    // Disable "t" option for now
                    // fprintf(stderr,"Invalid out_type:  try  'd', 't' or
                    // 'c'.");
            ...
            if (i < argc - 1) {
                if (!y.compare("n")) {
                    ...
                    fprintf(stderr,
                            "Invalid ut_type:  try  'n', 'j', 'k', or 's'.");
                    Exception::critical();
                }
                i++;
            } else {
            ...
            /*		}else if (!x.compare("--clust_size") || !x.compare("-s")){
                        if (i < argc-1){
                            ...
                            fprintf(stderr,"No clust_size integer
               specified.\n"); Exception::critical();
                        }
                    }else if (!x.compare("--rebuild_step_ratio") ||
               !x.compare("-r")){ if (i < argc-1){ int r = strtol(argv[i+1],
               NULL, 0);
                            //TreeBuilder.rebuildStepRatio = r;
                            i++;
                        }else{
                            fprintf(stderr,"No rebuild_step_ratio integer
               specified.\n"); Exception::critical();
                        }*/
...
std::string ArgumentHandler::getMethod() { return this->method; }
int ArgumentHandler::getNumThreads() { return this->threads; }

// RMH
float ArgumentHandler::getClusterCutoff() { return this->clusterCutoff; }

std::string ArgumentHandler::getInFile() { return this->inFile; }

std::string ArgumentHandler::getNJTmpDir() { return this->njTmpDir; }
...
FILE *ArgumentHandler::getOutpuFile() { return this->outFile; }
...
bool ArgumentHandler::argumentTest() { return true; }

bool ArgumentHandler::useSSE() { return this->SSE; }

bool ArgumentHandler::getPrintTime() { return this->printTime; }
