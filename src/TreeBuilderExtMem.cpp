    ...
    if (this->useCandHeaps) {
        if (this->candHeapList == NULL) {
            ...
            /*			if (TreeBuilder.verbose >= 2 &&
               candHeapListsize() > 0){ LogWriter.stdErrLogln("Cleared candidate
               heap list when K = " + (2 * K - nextInternalNode));
                                    }*/
...
TreeNode **TreeBuilderExtMem::build() {
    ...
    while (this->nextInternalNode <
                    ...
                    while (!h->isEmpty()) {
                        ...
                        /*							if (bh == NULL)
                           ...
                           null binary heap, for heap " + clA + ", " + clB);
                                                                                        //h.describeHeap();
                                                                                        throw
                           new Exception("null binary heap");
                                                                                }*/
                        if (which) {
        ...
        // values and possibly reset the candidates (every few iterations? when
        // exceed some size?)
        ...
        /*			if (minD == FLT_MIN) {
                                        throw new Exception("minD was not
           assigned correctly");
                                }*/
        if (newK == 2) {
        ...
        while (x < nextInternalNode) {
            ...
            // 2)/(float)1000000;
            // // this is the level of precision of input distances. No use
            ...
            //				instead of writing to the file, write to
            // an in-memory D buffer.  if that buffer is full, then write it to
            // disk D[ra][rb-ra-1] = tmp; System.err.println(ri + ", " + rx + ":
            // " + tmp + " (R[" + ri +"] = " + R[ri] + "), R[" + rx + "] = " +
            // R[rx]);
        ...
        if (stepsUntilRebuild == 0) {
            /*				if (TreeBuilder.verbose >= 3) {
               ...
               ("Resetting the clusters and corresponding PQs after " +
               (K-newK-1) + " iterations");
                                            }*/
    ...
    for (x = lastCandidateIndex; x >= 0; x--) {
        ...
        if (rj == -1 ||
            ri ==
                -1 /*leftovers from prior seqs redirected to this position*/) {
            candidatesActive[x] =
                false; // dead node ... can safely remove, 'cause
                       // we're going backwards through the list
            if (x == lastCandidateIndex) {
                // scan left to find it
    ...
    if (useCandHeaps) {
        ...
        if (usingSimpleCandidates) {
            ...
            if (candCnt >= 2000000 ||
                ...
                heap->insert(i, j, q);
                /*				} catch (Exception e) {
                                                        LogWriter.stdErrLogln("Death
                   while appending candidate.  nextInternalNode = " +
                   nextInternalNode); throw e;
                                                }*/
