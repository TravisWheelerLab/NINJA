...
TreeBuilderExtMem::TreeBuilderExtMem(std::string **names, int namesSize,
                                     float *R, std::string njTmpDir,
                                     FILE *diskD, float **memD,
                                     int memDFirstSize, int firstMemCol,
                                     int rowLength, long maxMemory) {
    this->candFilePos = 0;
    ...
    if (!useBinPairHeaps && !useCandHeaps) {
        Exception::critical();
        // throw new Exception ("external memory method must use one of the
        // heaps");
    }
...
void TreeBuilderExtMem::clusterAndHeap(int maxIndex) {
    ...
    if (this->useCandHeaps) {
        if (this->candHeapList == NULL) {
            ...
            /*			if (TreeBuilder.verbose >= 2 && candHeapListsize() > 0){
                            LogWriter.stdErrLogln("Cleared candidate heap list
               when K = " + (2 * K - nextInternalNode));
                        }*/
    ...
    if (!this->useBinPairHeaps) { // just using candidate heaps
        ...
        this->starterCandHeap = new CandidateHeap(
            this->njTmpDir, NULL, this->newK, this, this->maxMemory / 4);
        ...
        for (i = 0; i < this->clustCnt; i++) {
            for (j = i; j < this->clustCnt; j++) {
                if (this->arrayHeaps[i][j] != NULL) {
                    ...
                    this->arrayHeaps[i][j] = new ArrayHeapExtMem(
                        this->njTmpDir, this->redirect,
                        this->maxMemory /
                            666 /*that's about 3MB if mem is 2GB*/);
        ...
        while (j < maxIndex) {
            ...
            if (j >= this->firstColInMem) {
                ...
                if (j >= buffStart + memDSize) {
                    // read in next page;
                    while ((buffStart += memDSize) + memDSize <= j)
                        ; // probably just move to the next page
...
TreeNode **TreeBuilderExtMem::build() {
    ...
    // try {
    while (this->nextInternalNode <
           2 * this->K - 1) { // until there are 3 left ... at which point the
                              // merging is obvious
        ...
        if (!returnCandsToHeaps) {
            ...
            candidateViewsPerLoop[K - newK] = candidateCountPerLoop[K - newK] =
                lastCandidateIndex - inactiveCnt + 1;

            if (useCandHeaps) {
                for (int i = 0; i < (int)candHeapList->size(); i++) {
                    candidateCountPerLoop[K - newK] +=
                        candHeapList->at(i)->size();
                }
            }

            if (useBinPairHeaps) {
                /*	frequently (every 50 or 100 iters?), scan through the
                 * candidates, and return entries to the array heap that
                 * shouldn't be candidates any more
                 */
                int clI, clJ;
                if ((K - newK) % TreeBuilder::candidateIters == 0 &&
                    stepsUntilRebuild > TreeBuilder::candidateIters / 2) {
                    for (x = lastCandidateIndex; x >= 0; x--) {
                        if (!candidatesActive[x])
                            continue;
                        ...
                        maxTSum = maxT1val[clI] +
                                  (clI == clJ ? maxT2val[clI] : maxT1val[clJ]);
                        qLimit = candidatesD[x] * (newK - 2) - maxTSum;

                        if (qLimit > minQ) {
                            // it won't be picked as a candidate in next iter,
                            // so stick it back in the cluster heaps
                            removeCandidate(x);
                            ...
                            arrayHeaps[clA][clB]->insert(
                                candidatesI[x], candidatesJ[x], candidatesD[x]);
                        }
                    }
                }
            }
            ...
            if (lastCandidateIndex > 0 &&
                inactiveCnt > (float)lastCandidateIndex / 5) {
                int left = 0;
                int right = lastCandidateIndex;
                while (left < right) {
                    while (left < right && candidatesActive[left])
                        left++;
                    while (right > left && !candidatesActive[right])
                        right--;
                    if (left < right) {
            ...
            if (useCandHeaps) {
                ...
                for (int c = candHeapList->size() - 1; c >= 0;
                     c--) { // backwards through the list since more recent ones
                            // are likely to have bounded values closer to real
                            // values, so I want to use them to reduce the minQ
                            // first
                    cHeap = candHeapList->at(c);

                    cHeap->calcDeltaValues(newK);

                    // pluck entries off the heap as long as the delta-based
                    // condition allows -> stick them in the cand. list
                    while (!cHeap->isEmpty()) {
                        which = cHeap->getBinaryHeapWithMin().which;
                        if (which)
                            H2 = (BinaryHeap_FourInts *)cHeap
                                     ->getBinaryHeapWithMin()
                                     .h;
                        else
                            H = (BinaryHeap_TwoInts *)cHeap
                                    ->getBinaryHeapWithMin()
                                    .h;

                        if (which)
                            qPrime = H2->heap->front().key;
                        else
                            qPrime = H->heap->front().key;

                        candidateViewsPerLoop[K - newK]++;

                        if (cHeap->k_over_kprime * qPrime + cHeap->minDeltaSum <
                            minQ) { // "key" hold the q_prime value (q at time
                                    // of insertion to candidate list)
                            ...
                            ri = redirect[i];
                            rj = redirect[j];
                            cHeap->removeMin(); // remove the one we're looking
                                                // at
                            if (rj == -1 || ri == -1 /*leftovers from prior seqs redirected to this position*/) {
                                // don't bother to keep it for return to the
                                // minheap
                                defunct_cnt++;
                            } else {
                                // convert q' back to d;
                                d = (qPrime + cHeap->rPrimes[ri] +
                                     cHeap->rPrimes[rj]) /
                                    (cHeap->kPrime - 2);
                                q = d * (newK - 2) - R[ri] - R[rj];
                                //								q = cHeap.k_over_kprime *
                                //(qPrime + cHeap.rPrimes[ri] +
                                //cHeap.rPrimes[rj]) - R[ri] - R[rj];
                    ...
                    // if the remaining size of this heap is too small, mark
                    // this heap as needing to be cleared. if ( cHeap.size() /
                    // cHeap.representedRowCount < complexCandidateRatio ||
                    // cHeap.size() < cHeap.origSize * .5) { if ( cHeap.size() /
                    // cHeap.representedRowCount < (complexCandidateRatio/2) ||
                    // cHeap.size() < cHeap.origSize * .5) {
                    if (cHeap->size() <
                        cHeap->origSize *
                            candHeapDecay) { // by now, lots of these are likely
                                             // defunct anyway.  Reset.
                        cHeap->expired = true;
                        expiredHeapExists = true;
                    }
                }

                // empty the heaps needing to be cleared - the entries will be
                // merged into a new heap if appropriate limits are reached
                if (expiredHeapExists) {
                    for (int c = candHeapList->size() - 1; c >= 0; c--) {
                        ...
                        if (cHeap->expired) {
                            ...
                            // LogWriter.stdErrLogln("Removed a candidate heap
                            // (with K = " + cHeap.kPrime + ") when newK =" +
                            // newK );
                        }
                    }
                }
        ...
        if (!useBinPairHeaps && !starterCandHeap->isEmpty()) {
            // pull off entries from the primary candidate heap that have some
            // chance of having minQ
            float qPrime, d;
            BinaryHeap_TwoInts *H;
            BinaryHeap_FourInts *H2;
            bool which;
            ...
            // pluck entries off the heap as long as the delta-based condition
            // allows -> stick them in the cand. list
            while (!starterCandHeap->isEmpty()) {
                which = cHeap->getBinaryHeapWithMin().which;
                if (which)
                    H2 = (BinaryHeap_FourInts *)cHeap->getBinaryHeapWithMin().h;
                else
                    H = (BinaryHeap_TwoInts *)cHeap->getBinaryHeapWithMin().h;
                ...
                if (starterCandHeap->k_over_kprime * qPrime +
                        starterCandHeap->minDeltaSum <=
                    minQ) {
                    if (which) {
                        i = H2->heap->front().first;
                        j = H2->heap->front().second;
                    ...
                    rj = redirect[j];
                    starterCandHeap
                        ->removeMin(); // remove the one we're looking at
                    if (rj == -1 || ri == -1 /*leftovers from prior seqs redirected to this position*/) {
                        // don't bother to keep it for return to the minheap
                        defunct_cnt++;
                    } else {
                        // convert q' back to d;
                        d = (qPrime + starterCandHeap->rPrimes[ri] +
                             starterCandHeap->rPrimes[rj]) /
                            (starterCandHeap->kPrime - 2);
                        q = d * (newK - 2) - R[ri] - R[rj];
                        //								q = cHeap.k_over_kprime * (qPrime +
                        //cHeap.rPrimes[ri] + cHeap.rPrimes[rj]) - R[ri] -
                        //R[rj];
                        appendCandidate(d, i, j);
            ...
            // pull off entries for the bin-pair heaps that have some chance of
            // having minQ
            int h_i, h_j;
            float h_d;
            ...
            for (a = 0; a < clustCnt; a++) {
                for (b = a; b < clustCnt; b++) {
                    ...
                    clA = clustersBySize[a] < clustersBySize[b]
                              ? clustersBySize[a]
                              : clustersBySize[b];
                    clB = clustersBySize[a] < clustersBySize[b]
                              ? clustersBySize[b]
                              : clustersBySize[a];

                    maxTSum = maxT1val[clA] +
                              (clA == clB ? maxT2val[clA] : maxT1val[clB]);
                    ...
                    while (!h->isEmpty()) {
                        ...
                        if (which)
                            bh2 =
                                (BinaryHeap_FourInts *)h->getBinaryHeapWithMin()
                                    .h;
                        else
                            bh = (BinaryHeap_TwoInts *)h->getBinaryHeapWithMin()
                                     .h;

                        /*							if (bh == NULL) {
                                                        //LogWriter.stdErrLogln("Surprising:
                           null binary heap, for heap " + clA + ", " + clB);
                                                        //h.describeHeap();
                                                        throw new
                           Exception("null binary heap");
                                                    }*/
                        if (which) {
                        ...
                        //								System.err.println("D: " + h_i + ", "
                        //+ h_j + ": " + qLimit + " (" + q + ", " + maxTSum +
                        //")");

                        if (qLimit <= minQ) {
                            // it's possible that this or one of the following
                            // nodes on the heap has Q-value less than the best
                            // I've seen so far
                            ...
                            break; // no use pulling more off the heap ... they
                                   // can't beat the best so far.
                        }
                    }

                    if (returnCandsToHeaps) {

                        // log the number of candidates grabbed at each
                        // iteration. This tells us how the inner loop scales
                        // with the # seqs.
                        candidateViewsPerLoop[K - newK] +=
        ...
        // Now I know the position on the candidates array that has the best Q
        // node. Remove it from the candidates, merge the nodes, update D/T
        // values
        // and possibly reset the candidates (every few iterations? when exceed
        // some size?)
        ...
        // if a length is negative, move root of that subtree around to
        // compensate.
        ...
        //    	for all these D reads, should check if the i/j/x value is
        //    greater than the most recently written-to-disk index
        //   	 ... if not, then read from the memD, not diskD
        R[ri] = 0;
        long diskPos;

        // I need to get this before going into the "foreach x" loop, 'cause I
        // need Dij for all new vals.
        if (min_i >= firstColInMem) {
            ...
            //			    	diskPos = 4 * (numDCols *
            //(ri-(numDRowsPerFile*diskFile_i))  + h_j );
            fseek(diskD, diskPos, SEEK_SET);
        ...
        while (x < nextInternalNode) {
            ...
            if (min_i >= firstColInMem) {
                ...
                if (x >= buffStart_i + memDSize) {
                    // read in next page;
                    while ((buffStart_i += memDSize) + memDSize <= x)
                        ; // probably just move to the next page
            ...
            if (min_j >= firstColInMem) {
                ...
                if (x >= buffStart_j + memDSize) {
                    // read in next page;
                    while ((buffStart_j += memDSize) + memDSize <= x)
                        ; // probably just move to the next page
            ...
            // tmp =  Math.round(1000000 * (Dxi + Dxj - Dij) /
            // 2)/(float)1000000;	 // this is the level of precision of input
            // distances. No use allowing greater (noisy) precision to dominate
            // decisions
            tmp =
                (Dxi + Dxj - Dij) /
                2; // this is the level of precision of input distances. No use
                   // allowing greater (noisy) precision to dominate decisions

            R[ri] += tmp;
            R[rx] += tmp - (Dxi + Dxj);

            //				instead of writing to the file, write to an in-memory D
            //buffer.  if that buffer is full, then write it to disk
            // D[ra][rb-ra-1] = tmp;
            // System.err.println(ri + ", " + rx + ": " + tmp + " (R[" + ri +"]
            // = " + R[ri] + "), R[" + rx + "] = " + R[rx]);
        ...
        if (curColInMem == firstColInMem + memDSize) {
            ...
            // first, we append each row in memD to existing rows in the file
            // (one disk block per row)
            x = firstActiveNode;
            while (x < nextInternalNode) {
                rx = redirect[x];
                ...
                //						diskPos = 4 * (numDCols *
                //(rx-(numDRowsPerFile*diskFile)) + firstColInMem )   ;
                fseek(diskD, diskPos, SEEK_SET);
            ...
            // then write each column as a full new row in the file (which will
            // cover multiple blocks)
            float fBuff_horiz[nextInternalNode];
            int ry;
            for (i = firstColInMem; i < curColInMem;
                 i++) { // for each new column
                // write distances from the memD into a buffer,
                // which is then written to the row for this new node (ri)

                ry = redirect[i];
                if (ry == -1)
                    continue; // this node was already merged with another, even
                              // before being dumped to file

                x = firstActiveNode;
                while (x < nextInternalNode) {
                    // many entries in the buffer are left in default.
                    // minor speedup could be had in the conversion routine (but
                    // I won't bother)
                    rx = redirect[x];
        ...
        if (stepsUntilRebuild == 0) {
            /*				if (TreeBuilder.verbose >= 3) {
                                LogWriter.stdErrLogln ("Resetting the clusters
               and corresponding PQs after " + (K-newK-1) + " iterations");
                            }*/
            ...
            if (newK < 200) {
                stepsUntilRebuild =
                    newK; // almost done, quit shrinking the rebuild size
            } else {
                if (TreeBuilder::rebuildSteps == -1) {
                    if (TreeBuilder::rebuildStepsConstant)
                        stepsUntilRebuild =
                            (int)(K * TreeBuilder::rebuildStepRatio);
                    else
                        stepsUntilRebuild =
                            (int)(newK * TreeBuilder::rebuildStepRatio);
                } else {
            ...
            if (useBinPairHeaps) {
                // re-set the max levels for clusters (based on
                // one-iteration-old data)
                for (j = 0; j < clustCnt; j++) {
                    // LogWriter.stdErrLogln("updating cluster " + j + " from "
                    // + clustPercentiles[j] + " to "  + maxT1val[j]);
            ...
            // ... then add all new distances to the appropriate place (bin-pair
            // heap or candidates)
            x = firstActiveNode;
            float d;
            while (x < nextInternalNode) {
...
void TreeBuilderExtMem::returnCandidates() {
    ...
    for (x = lastCandidateIndex; x >= 0; x--) {
        ...
        if (rj == -1 ||
            ri ==
                -1 /*leftovers from prior seqs redirected to this position*/) {
            candidatesActive[x] =
                false; // dead node ... can safely remove, 'cause we're going
                       // backwards through the list
            if (x == lastCandidateIndex) {
                // scan left to find it
                while (x > 0 && !candidatesActive[x]) {
                    x--;
                }
            ...
            arrayHeaps[cra][crb]->insert(candidatesI[x], candidatesJ[x],
                                         candidatesD[x]);
        }
    }
    ...
    if (candFilePos > 0) {
        ...
        while (candFilePos > 0) {
            ...
            fseek(candFile, (candFilePos - numCandTriplesToDisk) * 3 * 4,
                  SEEK_SET);
...
void TreeBuilderExtMem::appendCandidate(float d, int i, int j) {
    ...
    if (useCandHeaps) {
        ...
        if (usingSimpleCandidates) {
            ...
            if (candCnt >= 2000000 ||
                (candCnt >= candHeapThresh &&
                 candCnt / newK > complexCandidateRatio)) {
                usingSimpleCandidates = false;
                CandidateHeap *heap = new CandidateHeap(
                    njTmpDir, NULL, newK, this,
                    maxMemory / 1000 /* roughly 2MB for 2GB RAM */);
                float q = 0;

                // try {
                for (int x = 0; x <= lastCandidateIndex; x++) {
                    if (candidatesActive[x] && redirect[candidatesI[x]] != -1 &&
                        redirect[candidatesJ[x]] != -1) {
                        q = candidatesD[x] * (newK - 2) -
                            R[redirect[candidatesI[x]]] -
                            R[redirect[candidatesJ[x]]];
                ...
                heap->insert(i, j, q);
                /*				} catch (Exception e) {
                                    LogWriter.stdErrLogln("Death while appending
                   candidate.  nextInternalNode = " + nextInternalNode); throw
                   e;
                                }*/
                ...
                int ri, rj;
                float qPrime;
                if (candHeapList->size() ==
                    100) { // don't let the number of heaps exceed 100 ... merge
                           // the oldest 20 into this one
                    for (int c = 0; c < 20; c++) {
                        expiringHeap = candHeapList->at(0);
                        while (!expiringHeap->isEmpty()) {
                            which = expiringHeap->getBinaryHeapWithMin().which;
                            if (which)
                                H2 = (BinaryHeap_FourInts *)expiringHeap
                                         ->getBinaryHeapWithMin()
                                         .h;
                            else
                                H = (BinaryHeap_TwoInts *)expiringHeap
                                        ->getBinaryHeapWithMin()
                                        .h;
                            ...
                            if (ri != -1 && rj != -1) {
                                ...
                                d = (qPrime + expiringHeap->rPrimes[ri] +
                                     expiringHeap->rPrimes[rj]) /
                                    (expiringHeap->kPrime - 2);
                                q = d * (newK - 2) - R[ri] - R[rj];
                                heap->insert(i, j, q);
                            }
                        ...
                        // if (TreeBuilder.verbose >= 2)
                        // LogWriter.stdErrLogln("Removed a candidate heap (with
                        // K = " + expiringHeap.kPrime + ") when newK =" + newK
                        // );
                    }
                }
                ...
                // if (TreeBuilder.verbose >= 2)
                // LogWriter.stdErrLogln("Added a candidate heap when newK =" +
                // newK + " : total heap count is " + candHeapList.size());
            ...
            d = d * (newK - 2) - R[redirect[i]] -
                R[redirect[j]]; // really q' now
...
int TreeBuilderExtMem::appendCandToSimpleList(float d, int i, int j) {
    ...
    if (freePos == this->candidatesSize) { // is it the right number?
        int newLength = (int)(this->candidatesSize *
                              (this->candidatesSize < 1000000 ? 10 : 2));
        ...
        if (newLength > 8000000) {
            ...
            // need to write some of the current entries to a file, and add
            // entries to freeCandidates try {
            int *iBuff = new int[numCandTriplesToDisk * 3];
            for (x = 0; x < numCandTriplesToDisk; x++) {
