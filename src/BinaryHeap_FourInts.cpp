...
bool compare4Ints(ints4float x, ints4float y) { return (x.key > y.key); }
...
BinaryHeap_FourInts::BinaryHeap_FourInts(int *val1s, const int *val2s,
                                         const int *val3s, const int *val4s,
                                         Float keys) {
    this->heap = new std::vector<ints4float>();
    this->heap->reserve(this->DEFAULT_CAPACITY);
    std::make_heap(this->heap->begin(), this->heap->end(), compare4Ints);
...
void BinaryHeap_FourInts::buildAgain(int *val1s, const int *val2s,
                                     const int *val3s, const int *val4s,
                                     Float keys) {
    makeEmpty();
    this->heap = new std::vector<ints4float>();
...
BinaryHeap_FourInts::~BinaryHeap_FourInts() { makeEmpty(); }

void BinaryHeap_FourInts::insert(int val1, int val2, int val3, int val4,
                                 float key) {
    ints4float aux;
    aux.first = val1;
    aux.second = val2;
...
bool BinaryHeap_FourInts::isEmpty() { return this->heap->empty(); }

int BinaryHeap_FourInts::size() { return this->heap->size(); }
...
int compareMyType4(const void *a, const void *b) {
    if (((ints4float *)a)->key < ((ints4float *)b)->key)
        return -1;
    if (((ints4float *)a)->key == ((ints4float *)b)->key)
        return 0;
    return 1;
    ...
    for (int l = 1; l < 6; l++) {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            for (int i = 0; i < 10000; i++) {
                ...
                aux->insert(int_aux[i].first, int_aux[i].second,
                            int_aux[i].third, int_aux[i].fourth,
                            int_aux[i].key);
            }
            ...
            if (verbose)
                printf(
                    "Binary Heap Four Ints Test %d completed successfully.\n",
                    k);
        }
    }
