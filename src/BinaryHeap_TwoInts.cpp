...
bool compare2IntsFloat(ints2float x, ints2float y) { return (x.key > y.key); }
...
BinaryHeap_TwoInts::BinaryHeap_TwoInts(int *val1s, const int *val2s,
                                       Float keys) {
    this->heap = new std::vector<ints2float>();
    this->heap->reserve(this->DEFAULT_CAPACITY);
    std::make_heap(this->heap->begin(), this->heap->end(), compare2IntsFloat);
    for (int i = 0; i < keys.length; i++) {
        ...
        std::push_heap(this->heap->begin(), this->heap->end(),
                       compare2IntsFloat);
    }
}
...
bool BinaryHeap_TwoInts::isEmpty() { return this->heap->empty(); }

int BinaryHeap_TwoInts::size() { return this->heap->size(); }
...
void BinaryHeap_TwoInts::chopBottomK(int *val1s, int *val2s,
                                     Float keys) { // no idea if it works
    int k = keys.length;
    int pos;
    for (int i = 0; i < k; i++) {
...
bool BinaryHeap_TwoInts::binHeapTwoTest(bool verbose) {
    ...
    for (int l = 1; l < 6; l++) {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            if (verbose)
                printf(
                    "Binary Heap Two Ints Test (Float Key) Test %d completed "
                    "successfully.\n",
                    k);
        }
    }
    printf(
        "Binary Heap Two Ints Test (Float Key) Test completed successfully.\n");
    return true;
}
