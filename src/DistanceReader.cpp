...
#pragma omp parallel for
    for (int i = 0; i < this->K; i++) {
        for (int j = i + 1; j < this->K; j++) {
            ...
            distances[i][j - i - 1] =
                this->distCalc->calc(i,
                                     j); // this gets the same rounding I have
                                         // in the distance writer code
        }
    }
...
float DistanceReader::atoi(char *in, int end) {
    ...
    while (pos >= 0) {
        if (in[pos] == '-') {
            ...
            fprintf(
                stderr,
                "Unable to convert integer from invalid char array. Position + "
                "%d, char = '%c', numberString = '%s'",
                pos, in[pos], in);
            Exception::critical();
        } else {
