...
TreeBuilderBinHeap::TreeBuilderBinHeap(std::string **names, int **distances,
                                       int namesSize)
    : TreeBuilder(names, distances, namesSize) {
...
void TreeBuilderBinHeap::clusterAndHeap(int maxIndex) {
    ...
    for (i = 0; i < this->clustCnt - 1; i++) {
        this->clustPercentiles[i] =
            minT + (i + 1) * seedTRange / this->clustCnt;
    }
    ...
    /*			if (verbose >= 4) {
            ...
            for (i=0; i<clustCnt; i++)
                    LogWriter.stdErrLogln(i + " : " + clustSizes[i] + " ( " +
    clustPercentiles[i] + " )");
    }*/

    //	make PQs for each cluster pair (make candidate list, too)  ... this will
    // allow old ones to be collected
    ...
    while (i < maxIndex) {
        ...
        while (j < maxIndex) {
            ...
            d = this->D[ra][rb - ra - 1];
            this->heaps[cra][crb]->insert(
                i, j, d); // this can be sped up for really big heaps by
                          // inserting all as one big list
...
TreeNode **TreeBuilderBinHeap::build() {
    ...
    while (nextInternalNode <
        ...
        for (x = this->lastCandidateIndex; x >= 0; x--) {
            ...
            if (rj == -1 ||
          ri == -1 /*leftovers from prior seqs redirected to this position*/) {
                this->candidatesActive[x] =
                ...
                if (x == this->lastCandidateIndex) {
                    ...
                    int y = x;
                    while (y > 0 && !this->candidatesActive[y]) {
                        y--;
                    }
                ...
                q = (long)this->candidatesD.pointer[x] * (newK - 2) - R[ri] -
                    R[rj];
                if (q <= minQ) {
                    //							if
                    //(nextInternalNode
                    //==
                    // 1150) System.out.println("(fib a) next node: " +
                    // nextInternalNode +
                    //", cand: " + candidatesI[x] + ", " + candidatesJ[x] + " (
                    // q=" +q + " , minQ=" + minQ +")");
        ...
        this->candidateCountPerLoop[this->K - newK] =
            this->lastCandidateIndex - inactiveCnt + 1;

        /*	frequently (every 50 or 100 iters?), scan through the candidates,
         * and return entries to the array heap that shouldn't be candidates any
         * more
         */
        if ((this->K - newK) % TreeBuilder::candidateIters == 0 &&
            ...
            for (x = this->lastCandidateIndex; x >= 0; x--) {
                ...
                qLimit =
                    (long)this->candidatesD.pointer[x] * (newK - 2) - maxTSum;

                if (qLimit > minQ) {
                    // it won't be picked as a candidate in next iter, so stick
                    // it back in the cluster heaps
                    ...
                    if (clI <= clJ)
                        this->heaps[clI][clJ]->insert(
                            this->candidatesI.pointer[x],
                            this->candidatesJ.pointer[x],
                            this->candidatesD.pointer[x]);
                    else
                        this->heaps[clJ][clI]->insert(
                            this->candidatesI.pointer[x],
                            this->candidatesJ.pointer[x],
                            this->candidatesD.pointer[x]);
                }
        ...
        if (this->lastCandidateIndex > 0) {
            if (inactiveCnt > (float)this->lastCandidateIndex / 5) {
                ...
                while (left < right) {
                    while (left < right && this->candidatesActive[left])
                        left++;
                    while (right > left && !this->candidatesActive[right])
                        right--;
                    if (left < right) {
                        this->candidatesD.pointer[left] =
                            this->candidatesD.pointer[right];
                        this->candidatesI.pointer[left] =
                            this->candidatesI.pointer[right];
                        this->candidatesJ.pointer[left] =
                            this->candidatesJ.pointer[right];
        ...
        // pull off entries for the fib-heaps that have some chance of having
        // minQ
        int h_i, h_j;
        int h_d;
        ...
        for (a = 0; a < this->clustCnt; a++) {
            for (b = a; b < this->clustCnt; b++) {
                ...
                clA = this->clustersBySize[a] < this->clustersBySize[b]
                          ? this->clustersBySize[a]
                          : this->clustersBySize[b];
                clB = this->clustersBySize[a] < this->clustersBySize[b]
                          ? this->clustersBySize[b]
                          : this->clustersBySize[a];
                ...
                while (!h->isEmpty()) {
                    ...
                    if (qLimit <= minQ) {
                        // it's possible that this or one of the following nodes
                        // on the heap has Q-value less than the best I've seen
                        // so far
                        h->deleteMin(); // pull it off

                        int pos = appendCandidate(h_d, h_i, h_j);
                        //								LogWriter.stdErrLogln("appended
                        //"
                        //+ heapNode.i
                        //+
                        //", " + heapNode.j + " to " + pos + " (" + q + ")");
        ...
        //				LogWriter.stdErrLogln("best was at " +
        // min_cand);
        ...
        if (newK == 2) {
            this->nodes[bestI]->length = nodes[bestJ]->length =
                (float)minD / 200000000;
        } else {
            this->nodes[bestI]->length =
                ((float)minD + (R[ri] - R[rj]) / (newK - 2)) / 200000000;
            this->nodes[bestJ]->length =
                ((float)minD + (R[rj] - R[ri]) / (newK - 2)) / 200000000;
        }
        ...
        /*		if (TreeBuilder.verbose >= 3) {
                                LogWriter.stdErrLogln("(inmem) next node: " +
           nextInternalNode + ": " + bestI + "(" + ri +") , " + bestJ + "(" + rj
           +
           ") Q = " + minQ + " (" + cand_cnt + " cands; " + defunct_cnt +  "
           defunct); "); LogWriter.stdErrLogln("     lengths: " +
        ...
        if (stepsUntilRebuild == 0) {
            ...
            /*			if (verbose >= 3) {
                                            LogWriter.stdErrLogln ("Resetting
               the clusters and corresponding PQs after " + (K-newK-1) + "
               iterations");
                                    }*/
            ...
            // re-set the max levels for clusters (based on one-iteration-old
            // data)
            for (j = 0; j < this->clustCnt; j++) {
            ...
            // nextActiveNode[prev] = nextInternalNode; // now this is the last
            // active node. prevActiveNode[nextInternalNode] = prev;
            this->redirect[nextInternalNode++] = ri;
        }
    }
    ...
    /*
            if (TreeBuilder.verbose >= 1) {
                    ...
                    LogWriter.stdErrLogln(defunct_cnt + " defunct nodes
       removed");
            }
    */
    ...
    /*	if (TreeBuilder.verbose >= 2) {
                    ...
                    LogWriter.stdErrLogln("max # candidates: " + max_cnt + " at
       iteration " + max_cnt_iter + " of " + candidateCountPerLoop.length);
                    LogWriter.stdErrLogln( "max ratio of candidates to possible
       pairs: " + String.format("%.7f", (float)max_ratio/100) + "%");

                    LogWriter.stdErrLogln("average # candidates: " +
       (sum_cnt/(K-4)) ); LogWriter.stdErrLogln("total # candidates: " + sum_cnt
       + "  ( of " + sum_possible + " possible)");

                    LogWriter.stdErrLogln("avg ratio of candidates to possible
       pairs: " + String.format("%.7f", ((float)sum_cnt/sum_possible)/100 ) +
       "%");
