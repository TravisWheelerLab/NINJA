...
TreeBuilderBinHeap::TreeBuilderBinHeap(std::string **names, int **distances,
                                       int namesSize, int *clusterEqual)
    : TreeBuilder(names, distances, namesSize, clusterEqual) {
...
TreeNode **TreeBuilderBinHeap::build() {
    ...
    while (nextInternalNode <
        ...
        for (x = this->lastCandidateIndex; x >= 0; x--) {
            ...
            if (rj == -1 || ri == -1 /*leftovers from prior seqs redirected to this position*/) {
                ...
                if (q <= minQ) {
                    ...
                    //								System.out.println("(fib a) next node: " +
                    //nextInternalNode + ", cand: " + candidatesI[x] + ", " +
                    //candidatesJ[x] + " ( q=" +q + " , minQ=" + minQ +")");
        ...
        if (stepsUntilRebuild == 0) {
            ...
            /*			if (verbose >= 3) {
                            LogWriter.stdErrLogln ("Resetting the clusters and
               corresponding PQs after " + (K-newK-1) + " iterations");
                        }*/
    ...
    */
    ...
    /*	if (TreeBuilder.verbose >= 2) {
       ...
       iteration " + max_cnt_iter + " of " + candidateCountPerLoop.length);
            LogWriter.stdErrLogln( "max ratio of candidates to possible pairs: "
       + String.format("%.7f", (float)max_ratio/100) + "%");

            LogWriter.stdErrLogln("average # candidates: " + (sum_cnt/(K-4)) );
            LogWriter.stdErrLogln("total # candidates: " + sum_cnt + "  ( of " +
       sum_possible + " possible)");

            LogWriter.stdErrLogln("avg ratio of candidates to possible pairs: "
       + String.format("%.7f", ((float)sum_cnt/sum_possible)/100 ) + "%");
