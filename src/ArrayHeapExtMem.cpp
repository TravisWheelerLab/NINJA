...
void ArrayHeapExtMem::initialize(std::string dir, int *activeIJs,
    ...
    this->numFieldsPerBlock =
        ...
        this->numFields; // note, this might not be blockSize, because of
                         // rounding.
...
int ArrayHeapExtMem::mergeLevels(int targetLevel, int *is, int *js,
    ...
    if (targetLevel > maxLevels - 1) {
        fprintf(stderr,
                "unexpected occurance: External Memory Array heap tried to "
                "write to a level > %d \n",
                maxLevels);
        Exception::critical();
    }
...
bool ArrayHeapExtMem::mergeSlots(int lvl) {
    ...
    if (lvl == maxLevels - 1) {
        /*		LogWriter.stdErrLogln("Request was made to merge two slots from
           ...
           maxLevels + ". " + "  That's not implemented yet!");*/
        Exception::critical();
        // return false;
    }
    ...
    // less than half full.
    // The Ferragina paper provides for doing all this at deletion time, which
    // allows a
    ...
    // are merged, the result of a merge
    // might already be small.  No use immediately turning around to merge that
    // one ... so here I merge
    // multiple slots - as many as will fit in one new slot.  Pick slots to
    // merge greedily
    slotPairList->clear();
    for (int slot = 0; slot < numSlots; slot++) {
        ...
        if (minSlot < 0) {
            ...
            /*			for (int i=0;i<numSlotsToMerge;i++){
               ...
               expired?"); LogWriter.stdErrLog("\t"); for (int slot2 : slots){
                                    LogWriter.stdErrLog("[ " + slot2 + " @ " +
               slotPositions[lvl][slot2] + " of " + slotNodeCnt[lvl][slot2] + "]
               , ");
                                }
                                LogWriter.stdErrLogln("");

                                throw new Exception ("Surprising minSlot ==
               -1.");
                            }
                        }*/
