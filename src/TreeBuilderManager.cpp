...
TreeBuilderManager::TreeBuilderManager(std::string method, std::string njTmpDir,
                                       std::string inFile, FILE *outFile,
                                       InputType inType, OutputType outType,
                                       AlphabetType alphType,
                                       CorrectionType corrType, int threads,
                                       bool useSSE, bool printTime) {
    this->method = method;
    ...
    */
    ...
    if (!this->method.compare("extmem")) {
        if (maxMemory < 1900000000) {
            fprintf(stderr, "Warning: using an external-memory variant of "
                            "NINJA with less than 2GB allocated RAM.\n");
            fprintf(stderr, "The data structures of NINJA may not work well if "
                            "given less than 2GB.\n");
        }
        ...
        fprintf(stderr,
                "created temporary directory for this run of NINJA : %s\n",
                njTmpDir.c_str());
        ...
        if (inType == alignment) {
            seqReader = new SequenceFileReader(
                &(this->inFile),
                (SequenceFileReader::AlphabetType)this->alphType);
            ...
            fprintf(stderr,
                    "External memory with distances as input not allowed.\n");
            Exception::critical();
        ...
        R = new float[K]();
        rowLength = (K + K - 2); // that's a K*K table for the initial values,
                                 // plus another K*(K-2) columns for new nodes
        ...
        if (numBlocks == 0)
            numBlocks = 1; // for huge inputs, this could result in memD larger
                           // than 400MB
        ...
        firstMemCol = reader->read(names, R, diskD, memD, numCols, rowLength,
                                   pageBlockSize);

        if (this->outType == dist) {
            fprintf(stderr,
                    "Output distances with external memory not allowed.\n");
            Exception::critical();
        }
        ...
        if (this->outType == dist) {
            ...
            if (this->inType != alignment) {
                fprintf(stderr, "Input and output distances not allowed. What "
    ...
    if (!this->method.compare("inmem") or !this->method.compare("default")) {
        ...
        tb_extmem =
            new TreeBuilderExtMem(names, K, R, njTmpDir, diskD, memD, numCols,
                                  firstMemCol, rowLength, maxMemory);
