...
SequenceFileReader::SequenceFileReader(std::string *filename,
    ...
    this->filetype = fasta;
    // TODO: If I assume a constant length throughout the sequences, it`s easy
    // to increase the performance of this read reads x and format the names and
    // sequences
    for (int i = 0; i < size; i++) { // try to read as a fasta format file
        if (x[i] == '>') {
            ...
            auxName.erase(remove_if(auxName.begin(), auxName.end(), isspace),
                          auxName.end());
            seq.push_back(auxName);
        } else {
            this->filetype = stockholm;
            break;
            fprintf(stderr, "Wrong formatting. Only fasta files are currently "
                            "accepted. Quitting\n");
            Exception::critical();
        }
    }
    ...
    //			}else{
    //				if(x[i]==' ' || x[i] == '\t' || x[i] == '\n')
    // i++; 				begin = i; 				while(x[i] != ' ' && x[i] !=
    // '\t'
    // && x[i] != '\n') 					i++;
    // end = i; 				std::string auxName;
    // auxName.assign(x +
    // begin*charSize,(end-begin)*charSize); 				if(auxName ==
    // "//") break; 				names.push_back(auxName);
    // while(x[i]!=' '&& x[i]!= '\t' && x[i] !=
    //'\n'){ 					i++;
    //				}
    //				i++;
    ...
    //				end = --i;
    //				auxName.assign(x +
    // begin*charSize,(end-begin)*charSize);
    //
    ...
    for (int i = 0; i < numSeqs;
         i++) { // get strings into a strings array instead of a vector
    ...
    if (this->alphType == null) {
        for (int i = 0; i < numSeqs; i++) {
            for (int j = 0; j < (signed)sequences[i]->size(); j++) {
                if (sequences[i]->at(j) != 'A' && sequences[i]->at(j) != 'G' &&
                    sequences[i]->at(j) != 'C' && sequences[i]->at(j) != 'T') {
                    this->alphType = amino;
                    break;
                }
            }
    ...
    for (int i = 0; i < numSeqs; i++) {
        // bool good = true;
        std::transform(sequences[i]->begin(), sequences[i]->end(),
                       sequences[i]->begin(), ::toupper);
        for (int j = 0; j < (signed)sequences[i]->size(); j++) {
            if (sequences[i]->at(j) == '.') {
                (*sequences[i])[j] = '-';
                // good = false;
            } else {
                if (this->alphType == dna &&
                    sequences[i]->at(j) == 'U') // change U to
                                                // T
                    (*sequences[i])[j] = 'T';
            }
        }
...
std::string **SequenceFileReader::getSeqs() { return this->seqs; }
std::string **SequenceFileReader::getNames() { return this->names; }
