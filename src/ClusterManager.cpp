...
ClusterManager::ClusterManager(std::string method, std::string njTmpDir,
                               std::string inFile, FILE *outFile,
                               InputType inType, OutputType outType,
                               AlphabetType alphType, CorrectionType corrType,
                               int threads, bool useSSE, bool printTime,
                               float clusterCutoff) {
    this->method = method;
...
std::string ClusterManager::doJob() {
    ...
    if (!this->method.compare("extmem")) {
        ...
        if (this->inType == alignment) {
            ...
            this->alphType =
                (ClusterManager::AlphabetType)seqReader->getAlphType();
        ...
        for (int m = 0; m < K - 1; m++) {
            ...
            int clust1 = 0;
            for (int i = 0; i < K - 1; i++) {
                // printf("Iterating i = %d, min_dist[%d] = %d,
                // distances[i][min_dist[i]] = %f\n", i, i, min_dist[i],
                // distances[i][min_dist[i]]);
                if (distances[i][min_dist[i]] <
                    distances[clust1][min_dist[clust1]])
                    clust1 = i;
            }
