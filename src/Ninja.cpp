...
int main(int argc, char *argv[]) {
    ...
    if (outType == ArgumentHandler::cluster) {
        ...
        ClusterManager *manager = new ClusterManager(
            method, njTmpDir, inFile, out, (ClusterManager::InputType)inType,
            (ClusterManager::OutputType)outType,
            (ClusterManager::AlphabetType)alphType,
            (ClusterManager::CorrectionType)corrType, threads, useSSE,
            printTime, clusterCutoff);

        std::string clusterString = manager->doJob();

    } else {
        TreeBuilderManager *manager =
            new TreeBuilderManager(method, njTmpDir, inFile, out,
                                   (TreeBuilderManager::InputType)inType,
                                   (TreeBuilderManager::OutputType)outType,
                                   (TreeBuilderManager::AlphabetType)alphType,
                                   (TreeBuilderManager::CorrectionType)corrType,
                                   threads, useSSE, printTime);
        ...
        if (!treeString.empty()) {
            ...
            fprintf(stderr, "\n\nTree string not generated for some unknown "
                            "reason. Aborting.");
            Exception::critical();
        }
