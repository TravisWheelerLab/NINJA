...
TreeBuilder::TreeBuilder(std::string **names, int **distances,
                         int namesSize) { // standard constructor
...
TreeBuilder::TreeBuilder(
    std::string **names, int **distances, int namesSize,
    int *clustersEqual) { // constructor that takes uses equal-clusters to
                          // speed-up clustering
    ...
    //		foundEqual = false;
    //		numLeft[i] = 0;
    ////		if (clustersEqual[i] != -1) //if this is linked to some cluster
    ///already, ignore it. this is necessary because if A == B and B == C, A ?=
    ///C. /			continue;
    //		for (j=i+1; j<this->K; j++) {
    //			if (this->D[i][j-i-1] == 0 && clustersEqual[j] == -1){ //equal
    //sequences and cluster not assigned yet 				if ((*this->seq[i]) ==
    //(*this->seq[j])){ //if the two sequences are exactly equal (necessary due
    //to non-transitivity of the relation) 					clustersEqual[j] = i;
    //					countClustered++;
    //					if (!foundEqual){
    //						numClusters++;
    ...
    if (numClusters > 0) { // there are equal sequences
        ...
        for (i = 0; i < this->K; i++) {
            ...
            if (rootsAdded < numClusters &&
                clusterRoots[rootsAdded] ==
                    i) { // if it is the root of an equal subtree I will put the
                         // subtree instead
            ...
            } else if (clustersEqual[i] ==
                       -1) { // should be used in the clustering
        ...
        for (size_t i = 0; i < nodesPos.size() - 1; i++) {
            for (size_t j = 0; j < nodesPos.size() - i - 1; j++) {
                ...
                // nodesPos[i+j+1]-i-1);
                this->D[i][j] =
                    this->D[nodesPos[i]][nodesPos[i + j + 1] - nodesPos[i] - 1];
            }
        }
        ...
        //				//now transform that to the right index by taking out k and
        //1 				memcpy(this->D[k][(shiftPos[i]-i)-k-1],
        //this->D[k][(shiftPos[i]-i)-k],
        //sizeof(int)*((this->K-k-1)-(shiftPos[i]-i)));
        //			}
        //		}
        ...
        for (i = 0; i < this->K; i++) {
            if (clustersEqual[i] != -1) { // if assigned to a cluster, I will
                                          // append it to the right node.
                TreeNode *newTree = new TreeNode(names[i]);
        ...
        // printf("Percentage of nodes taken due to by equality: %d",
        // countClustered*100/this->K);
...
void TreeBuilder::finishMerging() { // did not change anything expect the ->`s
                                    // instead of the .`s
