...
// TODO: Implement this
DistanceReaderExtMem::DistanceReaderExtMem(DistanceCalculator *distCalc,
                                           int K) {
...
int DistanceReaderExtMem::read(std::string **names, float *R, FILE *diskD,
                               float **memD, int memDRowsize, int rowLength,
                               int pageBlockSize) {
    int row = 0;
    ...
    // TODO: implement threads here, more complicated that in the in memory
    // function, but still feasible
...
#pragma omp parallel for
            for (int col = 0; col < numColsToDisk; col++) {
                ...
                float d = ((float)ceil(
                              (float)(10000000 * distCalc->calc(row, col)))) /
                          10000000;
